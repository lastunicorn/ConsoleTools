...
[TestFixture]
public class ContentRow_IsVisible_LastRow_Tests : TestsBase
{
    [Test]
    ...
    [Test]
    public void HavingFooterRowVisibleLastContentRowHidden_WhenRendered_ThenLastContentRowIsNotDisplayed()
    {
        DataGrid dataGrid = CreateDummyDataGrid();

        dataGrid.Rows[4].IsVisible = false;

        string expected = GetResourceFileContent("01-frow-row4-hidden.txt");
        dataGrid.IsEqualTo(expected);
    }

    [Test]
    public void HavingFooterRowMissingAndLastContentRowHidden_WhenRendered_ThenLastContentRowIsNotDisplayed()
    {
        DataGrid dataGrid = CreateDummyDataGrid(false);

        dataGrid.Rows[4].IsVisible = false;

        string expected = GetResourceFileContent("02-no-frow-row4-hidden.txt");
        dataGrid.IsEqualTo(expected);
    }

    private static DataGrid CreateDummyDataGrid(bool addFooter = true)
    {
